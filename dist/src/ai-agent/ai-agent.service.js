...
let AiAgentService = AiAgentService_1 = class AiAgentService {
    ...
    async onModuleInit() {
        ...
        try {
            ...
            const e = error;
            if (e?.code === 'P2021') {
                this.logger.warn('Skipping default AI agent initialization: table "ai_agents" does not exist yet. Run Prisma migrations.');
                return;
            }
            throw error;
        }
    ...
    async updateAgentStatus(id, isActive) {
        const updated = await this.prisma.aiAgent.update({
            where: { id },
            data: { isActive },
        });
    ...
    async updateAgentStats(id, conversationCount = 0, messageCount = 0) {
        return null;
    }
    async getBestAgent() {
    ...
    async initializeDefaultAgents() {
        ...
        const llmModel = this.configService.get('GEMINI_MODEL') || 'gemini-1.5-flash';
        const embeddingModel = this.configService.get('GEMINI_EMBEDDING_MODEL') || 'gemini-embedding-2-preview';
        const defaults = [
            {
                name: 'Text Agent',
                model: llmModel,
...
DO include facts like "prefers brief answers", "likes black clothing", "hates high shipping costs".`,
            ...
            if (!existing) {
                await this.prisma.aiAgent.create({
                    data: {
                        ...
                        model: d.model,
                        prompt: d.prompt,
                        instructionPrompt: d.instructionPrompt || null,
                        unavailableMessage: d.unavailableMessage || null,
                    }
                });
                this.logger.log(`  ✨ Created new agent: ${d.name}`);
            }
            else {
                const needsUpdate = existing.model !== d.model ||
                    existing.prompt !== d.prompt ||
                    existing.instructionPrompt !== (d.instructionPrompt || null) ||
                    existing.unavailableMessage !== (d.unavailableMessage || null);
                if (needsUpdate) {
    ...
    async getSystemPredefinedMessages() {
        ...
        if (!setting) {
            ...
            try {
                result = { ...defaults, ...JSON.parse(setting.value) };
            }
            catch {
                result = defaults;
            }
    ...
    async updateSystemPredefinedMessages(data) {
