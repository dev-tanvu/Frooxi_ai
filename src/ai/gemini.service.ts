...
export class GeminiService {
     ...
     * This output guides the Persona Brain (generateSalesResponse).
     */
...
User: ${prompt}
            ...
            // Attempt to parse JSON. Sometimes Gemini might still wrap it despite instructions.
            ...
            // Fallback to a neutral state so the bot doesn't completely break
            return {
                frustration_level: 0,
                intent: 'Browsing',
                urgency: 'Low',
...
User: ${prompt}
    ...
    /**
     * ORDER AGENT: Dedicated order collection response generator.
     * Uses the Order Agent's prompt and instructions to collect order details.
     */
    async generateOrderResponse(
        ...
        try {
            ...
            if (!agent) return 'Our ordering system is temporarily unavailable. Please try again shortly.';
...
User: ${prompt}
    ...
    async generateMultimodalEmbedding(text: string, imageBuffer: Buffer, mimeType: string, modelName?: string): Promise<number[]> {
        try {
            // Google AI Studio (GoogleGenerativeAI) doesn't support multimodal embeddings directly via .embedContent
            // Fallback: Generate a visual description and then embed the text.
            ...
            // Last resort: just use the text query if even that fails
     ...
     * Transcribes audio, analyzes intent, and generates the final response in a single step.
     */
